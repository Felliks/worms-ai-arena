...
class Lobby
{
    ...
    server_init(socket, io)
    {
        ...
        socket.on(Events.lobby.CREATE_GAME_LOBBY, (data) =>
        {
            ...
            socket.get('userId', (err, userId) =>
            {
                socket.get('profileUserId', (err, profileUserId) =>
                {

                    io.log.info(Util.format("@ Create lobby by user with ID [%s] with name  [%s] using map ", data.name, profileUserId, data.mapName));
                    var newGameLobby = this.server_createGameLobby(data.name, parseInt(data.nPlayers), data.mapName);
                    newGameLobby.join(userId, profileUserId, socket);
        ...
        socket.on(Events.gameLobby.PLAYER_JOIN, (gamelobbyId) => {
            ...
            socket.get('userId', (err, userId) =>
            {
                socket.get('profileUserId', (err, profileUserId) =>
                {
                    ...
                    gamelobby.join(userId, profileUserId, socket);
